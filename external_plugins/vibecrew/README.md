...
| **`assistant` agent** | The **operator's guide** (not host-ticked — a conversation the operator drives). Answers from the documentation, explains VibeCrew's processes, performs configuration and pipeline setup over the REST API, and runs **diagnostics** on request (fleet snapshot, run logs via `run-logs`, the API failure log via `log show`). Its write boundary is the plugin's tightest: **the configuration and the pipeline catalog are the only things it may write on its own** (`GET`/`PUT /api/config`; `GET /api/pipelines[/:name]` + `PUT /api/pipelines/:name`); board hygiene (card status moves, unused-workspace deletion) and filing an issue on the public tracker (`issue-create`, `dexloom/vibecrew_sh` — creation only) are **on-explicit-request** surfaces; every other endpoint read-only; no file-write tools in its grants. An OpenCode twin lives at `agents-opencode/va-assistant.md`. |
