...
| **`assistant` agent** | The **operator's guide** (not host-ticked — a conversation the operator drives). Answers from the documentation, explains VibeCrew's processes, and performs configuration and pipeline setup over the REST API. Its write boundary is the plugin's tightest: **the configuration and the pipeline catalog are the only things it may write** (`GET`/`PUT /api/config`; `GET /api/pipelines[/:name]` + `PUT /api/pipelines/:name`; every other endpoint read-only; no file-write tools in its grants). An OpenCode twin lives at `agents-opencode/va-assistant.md`. |
