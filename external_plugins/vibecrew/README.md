...
| **`product-manager` skill** | Turns a rough brief into a dev-ready VibeCrew card (spec → card), classifies it via `classify-task`, attaches the routed pipeline by default — the `## Pipeline` block is composed by the app through `POST /api/pipelines/:name/compose` (`vibecrew_api.py pipeline-compose`), so a plugin-filed card is byte-identical to one filed in the UI, and the returned `extension_metadata` is PATCHed onto the card — and decomposes multi-card briefs into **lanes** (epic + sub-cards + `blocking` edges). |
| **`classify-task` skill** | Main-agent-first routing resolved from the executor (Claude Code / OpenCode / Codex / Pi), then a five-axis tier (trivial/light/medium/heavy) → pipeline type (`Basic` / `Planned` / `Async`) + the default per-step agent and model bindings + stage toggles + the `**Routing:**` line. A step may run on a different agent than the main loop, and an operator naming an agent or model *for a step* is a first-class override. Telemetry-grounded; design record in `reference/routing.md`. **Pi** is an explicit-ask-only, uncalibrated arm — never auto-routed, as a main loop or as a step; **Codex** is uncalibrated but IS auto-routed once the executor ladder resolves to it, and is the default `plan-review`/`code-review` agent. |
...
| **`orchestrator` agent** | The **host-ticked board driver** (launched as the session agent via `claude --agent`; no `model:` pin — the launcher's per-agent model pick governs). It arms **no timer of its own**: VibeCrew's runtime owns the loop and delivers each tick as a prompt carrying a host-computed status digest — see [`reference/tick-contract.md`](reference/tick-contract.md). Every tick it reflects card status forward (`inreview`/`done`, per a delivery-signal gate honest about VibeCrew's asymmetric PR-vs-merge corroboration), **closes finished workspaces** (delete only on done + corroborated delivery + a terminal run; otherwise archive, loudly), dispatches ready cards, surfaces parked and stalled agents, applies opt-in directives, and ends its report with a `CADENCE:` line the host obeys (5m active ↔ 30m idle). All of it over the bundled client — no MCP tools, no subagent for the routine tick. It spawns `Agent(vibecrew:decider)` only for a direct "answer that questionnaire" request; a "create a card" instruction is bounced back to the operator — card creation stays operator-driven via `product`/`product-manager`. Byte-identical OpenCode and Codex twins live at `agents-opencode/vc-orchestrator.md` and `agents-codex/vc-orchestrator.md`. |
| **`assistant` agent** | The **operator's guide** (not host-ticked — a conversation the operator drives). Answers from the documentation, explains VibeCrew's processes, performs configuration and pipeline setup over the REST API, and runs **diagnostics** on request (fleet snapshot, run logs via `run-logs`, the API failure log via `log show`). Its write boundary is the plugin's tightest: **the configuration and the pipeline catalog are the only things it may write on its own** (`GET`/`PUT /api/config`; `GET /api/pipelines[/:name]` + `PUT /api/pipelines/:name`); board hygiene (card status moves, unused-workspace deletion), filing an issue on the public tracker (`issue-create`, `dexloom/vibecrew_sh` — creation only), and **repository maintenance** (`workspace-repos` state read + `commit`/`push`/`discard` on a workspace — server-performed, liveness-gated, never forced) are **on-explicit-request** surfaces; every other endpoint read-only; no file-write tools in its grants. OpenCode and Codex twins live at `agents-opencode/va-assistant.md` and `agents-codex/va-assistant.md`. |
| **`auditor` agent** | The **compliance reviewer** (not host-ticked — every turn is a question, from Crew Deck or `POST /api/auditor/ask`, which only delivers into the live session and never spawns). Pulls a card's audit bundle (`card-audit`: spec, plan, finalization record — shipping report + merges + PRs — per-commit changed files and diffs, deterministic checks) and judges whether what landed on main matches what the card asked for; findings land as card comments (`--kind auditor`). On request it lists (`audit-unused-workspaces`) and deletes unused workspaces, and moves cards — its ONLY write surfaces; no subagent tool, no file-write tools, no git mutations. OpenCode and Codex twins live at `agents-opencode/va-auditor.md` and `agents-codex/va-auditor.md`. |
| **`pm` agent** | The **intake PM** — the Product host agent in Crew Deck (not host-ticked; every turn hands it a brief, GitHub links, or a plan link). Runs the Product Skills (`product-manager`, `classify-task`) as its method: spec inline, classify + route, pipeline block composed by the SERVER (`pipeline-compose`, so a plugin-filed card and a UI-filed card are byte-identical), card filed, and multi-deliverable work arranged as lanes (parent epic + sub-cards + `blocking` edges, `orchestrate` only on an explicit auto-drive ask). GitHub links decompose into cards; plan links decompose into cards or file ONE card on a pipeline with the `plan` stage ticked. Writes cards and pipeline blocks, never code; never launches anything. The app's catalog (`crew-bundle/`) ships it standalone as `vibecrew-pm` — distinct from the `vibecrew-product` spec-writer subagent — and ships the two Product Skills as standalone skill entries the app installs alongside it. OpenCode and Codex twins live at `agents-opencode/vp-pm.md` and `agents-codex/vp-pm.md`. |
...
- Agents: `orchestrator`, `assistant`, `auditor`, `pm`, `product`, `planner`,
  ...
  `auditor`, and `pm` are operator-driven host agents (never ticked — the
  assistant answers, the auditor audits, the `pm` turns briefs, GitHub
  links, and plan links into routed, chained cards); `product`/`planner` are
...
- The app's own catalog (`crew-bundle/` — what VibeCrew installs from its
  `~/.vibecrew/plugins` checkout) additionally ships the host agents
  standalone (`vibecrew-orchestrator`, `vibecrew-assistant`,
  `vibecrew-auditor`, `vibecrew-pm` — the `pm`, distinct from the
  `vibecrew-product` spec-writer subagent) and the two Product Skills
  (`product-manager`, `classify-task`) as standalone skills, refreshed from
  this plugin's sources by `scripts/sync-crew-bundle.sh`.
