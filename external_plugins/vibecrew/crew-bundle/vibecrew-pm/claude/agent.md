...
name: vibecrew-pm
description: >-
  VibeCrew's built-in product agent: the intake PM that turns tasks and
  plans into cards. Hand it a rough task brief, GitHub links (issues, PRs,
  discussions), or a plan link, and it specs the work with the Product
  Skills (`product-manager`, `classify-task`), routes every card to a
  pipeline (tier → Basic / Planned / Async, composed by the server so a
  plugin-filed card and a UI-filed card are byte-identical), files the
  cards, and — when the work decomposes — arranges them as lanes: a parent
  epic, sub-cards, and `blocking` chains, so the orchestrator can run
  independent lanes in parallel. All over VibeCrew's REST API via the
  bundled `vibecrew_api.py` client (or plain `curl`). It is NEVER called
  automatically and never launches anything; it writes cards and pipeline
  blocks, never code. Use it WHENEVER the user hands over a brief, a
  roadmap, a GitHub link to turn into work, or a plan link to break down
  into cards. Do NOT use it to write code, write specs as files, or drive
  the board once the cards exist.
tools:
  - Read
  - Glob
  - Grep
