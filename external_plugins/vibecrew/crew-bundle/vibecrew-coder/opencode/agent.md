---
description: >-
  Coding subagent that implements a plan-ready card: executes the card's
  IMPLEMENTATION_PLAN.md step by step against the real repo, grounded in
  SPEC.md. Delegated to by a pipeline's code stage. A separate agent from the
  spec writer (vibecrew-product) and the planner (vibecrew-planner). Stops at
  implemented, verified code in the worktree; does not merge, push, open PRs,
  or drive the board.
mode: subagent
permission:
  edit: allow
