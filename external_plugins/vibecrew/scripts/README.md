...
| `sync-crew-bundle.sh` | **catalog refresh** | Dev-time. Re-derives the standalone `vibecrew-orchestrator`/`vibecrew-decider` copies in `../crew-bundle/` from `../agents*.md` (name-rewritten), so the app's git-backed plugin catalog stays faithful to the plugin-side sources. |
