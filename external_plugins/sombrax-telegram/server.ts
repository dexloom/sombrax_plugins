...
import { readFileSync, writeFileSync, mkdirSync, readdirSync, rmSync, statSync, renameSync, realpathSync, chmodSync, existsSync, appendFileSync } from 'fs'
...
mcp.setNotificationHandler(
  ...
  async ({ params }) => {
    ...
    if (listenerSocket) {
      // Client mode — forward to listener, which handles Telegram UI + callbacks
...
if (CLIENT_MODE) {
  // ================================================================
  // CLIENT MODE — connect to listener.ts via Unix socket
  // ================================================================
  ...
  if (!existsSync(LISTENER_SOCKET_PATH)) {
    process.stderr.write(
      `telegram channel: listener socket not found at ${LISTENER_SOCKET_PATH}\n` +
      `  start the listener first: bun listener.ts\n`,
    )
    process.exit(1)
  }
  ...
  listenerSocket.on('data', chunk => {
    lsBuf += chunk.toString()
    let nl: number
    while ((nl = lsBuf.indexOf('\n')) !== -1) {
      const line = lsBuf.slice(0, nl)
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'inbound': {
            ...
            if (msg.message_thread_id) meta.message_thread_id = msg.message_thread_id
            if (msg.image_path) meta.image_path = msg.image_path
            if (msg.attachment_kind) meta.attachment_kind = msg.attachment_kind
            if (msg.attachment_file_id) meta.attachment_file_id = msg.attachment_file_id
            if (msg.attachment_size) meta.attachment_size = msg.attachment_size
            if (msg.attachment_mime) meta.attachment_mime = msg.attachment_mime
          ...
          case 'session_reset': {
            // /new command was issued in Telegram — tell Claude to start fresh
            const resetMeta: Record<string, string> = {
              chat_id: msg.chat_id,
              user: msg.user,
              user_id: msg.user_id,
            }
        ...
        process.stderr.write(`telegram channel: bad message from listener: ${e}\n`)
      }
    }
  })

  listenerSocket.on('error', err => {
    process.stderr.write(`telegram channel: listener connection error: ${err}\n`)
    process.exit(1)
  })

  listenerSocket.on('close', () => {
    if (!shuttingDown) {
      process.stderr.write('telegram channel: listener disconnected\n')
      process.exit(1)
    }
  })
  ...
  // ================================================================
  // STANDALONE MODE — existing behavior: poll Telegram directly
  // ================================================================
