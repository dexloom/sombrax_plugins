...
| **`classify-task` skill** | Routes a task once, at card creation: **family first** (OpenCode vs Claude Code from the executor — never mixed: OpenCode = MiniMax/GLM/Kimi, Claude Code = Sonnet/Opus/Fable, Codex reviews both), then a five-axis rubric (scope, decisions, risk, novelty, verification) → complexity tier **trivial / light / medium / heavy** → per-family pipeline (**Quick / Async Sonnet / Async Opus / Async OpenCode GLM**; Async Fable explicit-ask only), stage toggles (spec adopt-vs-write, plan-review yes-vs-**gate**, code-review, merge-vs-PR — merge default-on), and the one-line `**Routing:**` record stamped on the card. The coder model binds **after the plan** (PLAN-GATE / CODER-MODEL late binding). Single source of truth for the family rule, rubric, and maps; an operator-named pipeline always overrides. Design record + evidence: `reference/routing.md`; the runtime tripwire is the `VK-ESCALATE` park marker (`CLAUDE.md`). |
